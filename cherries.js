...
filter = autoCurry(function(fn, xs){
...
reject = autoCurry(function(fn, xs){
...
partition = autoCurry(function(fn, xs){
  if (xs.length === +xs.length) {
...
takeWhile = autoCurry(function(fn, xs){
  var result = [], i = -1, len = xs.length;
  while (++i < len)
    if (fn(xs[i]))
      result.push(xs[i]);
...
dropWhile = autoCurry(function(fn, xs){
...
breakList = autoCurry(function(fn, xs){
  var a = [], b = [], i = -1, len = xs.length, drop = true;
  while (++i < len) {
    if (drop) {
      if (fn(xs[i]))
...
foldl = autoCurry(function(fn, memo, xs){
  var i = -1, len = xs.length;
  while (++i < len)
    memo = fn(memo, xs[i]);
  return memo;
...
foldl1 = autoCurry(function(fn, xs){
  // TODO
}),

foldr = autoCurry(function(fn, memo, xs){
  var i = xs.length;
  while (i--)
    memo = fn(memo, xs[i]);
  return memo;
...
foldr1 = autoCurry(function(fn, xs){
  // TODO
}),
...
sortBy = autoCurry(function(fn, xs){
...
lt = autoCurry(function(n, x) { return x < n }),
gt = autoCurry(function(n, x) { return x > n }), 
eq = autoCurry(function(n, x) { return x === n }),
lteq = autoCurry(function(n, x) { return x <= n }), 
gteq = autoCurry(function(n, x) { return x >= n }),
...
zipWith = autoCurry(function(fn, xs, ys){
  var result = [], i = -1, len = Math.min(xs.length, ys.length);
  while (++i < len)
...
listsToObj = autoCurry(function(xs, ys){
  var result = {}, i = -1, len = Math.min(xs.length, ys.length);
  while (++i < len)
    result[xs[i]] = ys[i];
  return result;
...
applyl = autoCurry(function(fn, args) {
  return function() {
    return fn.apply(this, args.concat(Array.prototype.slice.call(arguments)));
  };
}),

applyr = autoCurry(function(fn, args) {
  return function() {
